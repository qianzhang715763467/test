	...
	var Scroll = $.Class.extend({
		...
		_flick: function(e) {
			...
			if (this.options.momentum && detail.flickTime < 300) {
				...
				console.log("this.y						(当前对象Y轴滑动时的偏移量？)			:	"+this.y);
				console.log("flickDistanceY				(滑动距离值？)						:	"+detail.flickDistanceY);
				console.log("flickTime 					(当前touch动作耗时毫秒数)				:	"+detail.flickTime);
				console.log("this.maxScrollY 			(最大滚动值 == 可滚动区域的高度) 			:	"+this.maxScrollY);
				console.log("this.options.bounce ？ this.wrapperWidth ：0					:	"+this.wrapperWidth);
				console.log("this.options.deceleration 	(阻尼) 								:	"+this.options.deceleration);
				console.log("-----------------------------  callback    -----------------------------------");
				console.log("duration 					(滚动执行时间)								:	"+momentumY.duration);
				console.log("destination 				(计算后页面滚动值)					:	"+momentumY.destination);
				console.log("====================================");
		...
		_momentum: function(current, distance, time, lowerMargin, wrapperSize, deceleration) {
			var speed = parseFloat(Math.abs(distance) / time),		// (滑动距离的绝对值 / 当前touch动作耗时毫秒数) = 每毫秒滑动距离
				destination,
				duration;

			deceleration = deceleration === undefined ? 0.0006 : deceleration; // 默认值为0.0006
			// ？ = 滑动距量 + ( 毫秒滑动值 * 毫秒滑动值 ) / ( 2 * 阻尼系数 ) * 滑动方向  ;
