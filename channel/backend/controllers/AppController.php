...
class AppController extends Controller{
    ...
    public function actionApply(){
        ...
        if(Yii::$app->request->isPost){
            //进行校验
            $url = Yii::$app->params['channel_domain'].'api/v1/apply/verification';
            $curl = new curl;
            $params = [
            ...
            if($responseData['code']==200){
                //取出对应的账号信息
                $url = Yii::$app->params['channel_domain'].'api/v1/apply/apply';
                $curl = new curl;
                $params = [
                    'account_id' => Yii::$app->request->post('account_id'),
                ];
    ...
    public function actionSubmit(){
        if(Yii::$app->request->isPost){
            $url = Yii::$app->params['channel_domain'].'api/v1/apply/submit';
            $curl = new curl;
    ...
    public function actionFax(){
        ...
        if(Yii::$app->request->isPost){
            ...
            $responseData=json_decode($response,1);
            if($responseData['code']==200){
                $url = Yii::$app->params['channel_domain'].'api/v1/apply/fax';
                $curl = new curl;
                $params = [
    ...
    public function actionAgent(){
        if(Yii::$app->request->isPost){
            ...
            $url = Yii::$app->params['channel_domain'].'api/v1/agent/addlevel2';
            $curl = new curl;
            $params = Yii::$app->request->post();
            $response = $curl->setRequestBody(json_encode($params))
                ->setHeaders([
