            ...
            if ($model->hasErrors())
            {
                ...
                if($returnData['flag']==true){
                    ...
                    if(\common\lib\Common::updateBalance($requestData['account_id'])){
                        ...
                        $balance = \common\models\Balance::find()->where(['account_id'=>$requestData['account_id']])->asArray()->one();
//                        writeLog($requestData['amount'].'>>>>'.$balance['balance_amount']);
                        if($requestData['amount'] > $balance['balance_amount']){
                            $this->_responseData([],'余额不足',501);
                        }
                    }
