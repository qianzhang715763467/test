...
class OrderController extends ActiveController
{
    ...
    public function actionCreateprofit(){
        ...
        if( $requestData ){
            ...
            ->addRule('end_time','required')
            ->addRule('end_time','number')
            ->validate();
            if ($model->hasErrors()) {
                // 验证失败
...
//debug($profitInfo);                
                ...
                try{
                    ...
                    $profitModel->end_time = $requestData['end_time'];
...
//        writeLog($phpData);
        ...
        if( $requestData ){
            //数据验证
            $model = new DynamicModel( $requestData );
            $model->addRule('order_no','required')->validate();
            if ($model->hasErrors()) {
                // 验证失败
                $errorInfo = $model->firstErrors;
                $errorInfo = array_values($errorInfo);
                ...
                $transaction = Yii::$app->db->beginTransaction();
                try{
                    Yii::$app->db->createCommand()->delete('channel_profit', ['order_no'=>$requestData['order_no']])->execute();
                    Yii::$app->db->createCommand()->delete('channel_orders', ['order_no'=>$requestData['order_no']])->execute();
                    $accountingList = Accounting::find()->where(['doc_no'=>$requestData['order_no']])->all();
                    foreach( $accountingList as $row){
                        $accountId = $row->account_id;
                        $row->delete();
                        \common\lib\Common::updateBalance($accountId);
                    }
                    $transaction->commit();
                    writeLog('操作成功');
                    $this->_responseData([],'操作成功',200);
                }catch(\Exception $e){
                    $transaction->rollBack();
                    writeLog('事务回滚删除失败');
                    $this->_responseData([],'事务回滚删除失败',500);
                } 
            }
        }else{
            $this->_responseData([],'操作失败，请稍后再试',500);
    ...
    protected function setProfit($orderNo,$orderPrice,$pricePercent,$c1AccountId = '',$agentInfo = ''){
        ...
        if( empty($c1AccountId) ){
            ...
            $returnData['up_account_id'] = Yii::$app->params['lanye_account_id'];
            return $returnData;
        }else{
            if( empty($agentInfo) ){
