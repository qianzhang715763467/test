...
//        $end = urlencode("2017-04-12 16:42:24");
    ...
    public function testQueryMetricValueAction(){
        
        $chart_type = $this->request->get('chart_type');
        $metric_name = $this->request->get('metric_name');
        
        $sql = "select * from datax.$chart_type as a where a.i_time=(SELECT MAX(i_time) from datax.chart_common where name=?);";
        $conn = MySqlDB::getConnection();
        $values = $conn->fetchAll($sql, Db::FETCH_ASSOC, [$metric_name]);
