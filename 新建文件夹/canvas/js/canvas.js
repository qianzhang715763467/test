...
function drawCanvas(id,data){
	...
	this.draw = function(){
		...
		for(var i = 0;i < self.data.length;i++){
			...
			// 文字默认样式
			if(!self.data[i].font){
				self.data[i].font  = ['100 12px arial black'];
			}
			// 原点
	...
	this.animation = function(x1){// x1 = 对应的ata[x1]数据，x2 = 对应 data[x1].path[x2]
		...
		for(var i = 0;i < b.path.length;i++){
				...
				if(!obj.an || obj.an == "0" || obj.an == "false"){
					...
					objs.data[i]["size"]  = 0.02;// 起点到终点的绘制长度；
		...
		function timer (obj,key,index){
			var random = Math.random()*5 + parseInt(obj[index][key].data[key].sp); // 动画执行时间
	...
	this.arc = function(ctx,obj,animate){
		...
		if(animate){
			...
			if(!obj.an || obj.an == "0" || obj.an == "false"){
				...
				// 圆环 有动画
				ctx.arc(obj.x,obj.y,obj.r,obj.count*Math.PI,(obj.count+obj.size)*Math.PI,ot);
				obj.count += 0.01;
			}
			ctx.lineCap = obj.lc;
			ctx.lineWidth 	= obj.w;
	...
	this.border = function(ctx,obj){
		for(var i = 0; i < obj.path.length; i++){
			...
			ctx.lineWidth = objs.w;
			// 颜色数组长度 == 线段数组长度 ?
			if(obj.color.length == obj.path.length){
				ctx.strokeStyle = objs.c;
			}else{
				// 如果不相等只使用第一个值; 
	...
	this.line = function(ctx,obj){
		for(var i = 0;i < obj.angle.length; i++){
			...
			obj.angle[i] = 360 - 180*obj.angle[i];//第一段线段角度，0~2
			// 字体是否设置
			var w = this.fontW(objs,i);//根据字体宽度设置第二段线的宽度
			if(!obj.centerDot){ // 未设置原点？
				obj.coord = this.getCoord(objs.x,objs.y,objs.minR,objs.maxR,obj.angle[i],w); // X,Y原点、最小半径、最大半径、角度,字体宽度；
			}else {
	...
	// 文字
	this.Test = function(ctx,data){
		// w ： 文字宽度的一半；
		var w = this.fontW(data);
		ctx.beginPath();
		ctx.font 	  = data.f;
		ctx.fillStyle = data.c;
		ctx.fillText(data.t , data.x-w , data.y);
		ctx.closePath();
	};
	// 计算文字宽度，用于居中放置
	this.fontW = function(data){
	...
	this.escape = function(data,i,type){
		...
		// 转义
		var arr = ["path","radii","width","lineLength","centerDot","font"];
		for(var n = 0;n < arr.length; n++){
			...
			if(!data[arr[n]] && arr[n] != "centerDot"){
				...
				if(!data.font[n])data.font.push("100 12px arial black");// 原点未设置?设置默认值
				if(!data[arr[n]])continue;
	...
	this.replace = function(obj,Dot){
		if(obj){
			return	obj = obj.replace(/\h/g,id.offsetHeight).replace(/w/g,id.offsetWidth);
		}else{
			// 如果未设置原点，默认居中
			if(Dot){
